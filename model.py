"""
This file is contains different Model classes for training 
...
class DeepSpeech(nn.Module):
    def __init__(self, rnn_type=nn.LSTM, labels="abc", rnn_hidden_size=768, nb_layers=5, audio_conf=None,
        ...
        num_classes = len(self.labels)
          ## CNN Part feature extraction spectrogram 
        ...
        rnn_input_size *= 32
        ## RNN fpr Sequence to sequence 
        ## create RNN hidden lyaers
        rnns = []
        ...
        for x in range(nb_layers - 1):
            rnn = BatchRNN(input_size=rnn_hidden_size, hidden_size=rnn_hidden_size, rnn_type=rnn_type,
                           bidirectional=bidirectional) #RNN Parameters
            rnns.append(('%d' % (x + 1), rnn))
            ## adding RNNs tp the convoultional
